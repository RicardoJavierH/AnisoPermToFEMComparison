...
void ReadEntry(ProblemConfig &config, PreConfig &preConfig){
    ...
    switch(preConfig.type){
        ...
        case 3:
            ...
            break;
        case 4:
            config.exact.operator*().fExact = TLaplaceExample1::EAnisoPerm;
            break;
        default:
            DebugStop();
            break;
...
void EvaluateEntry(int argc, char *argv[],PreConfig &pConfig){
    ...
    if(argc == 11){
        ...
        else if(std::strcmp(argv[1], "EAnisoPerm") == 0) {
            pConfig.type = 4;
            pConfig.problem = "EAnisoPerm";
        }
        else DebugStop();
        ...
        else if (pConfig.problem == "EAnisoPerm") pConfig.type = 4;
        else DebugStop();
    }
