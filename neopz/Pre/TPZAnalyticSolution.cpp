...
//                x0[i] = TVar(-0.05);
        ...
        case EAnisoPerm:
        {
            ...
            TVar sigma = std::sqrt(0.005);
            
            //const TVar coef1 = 2/(3*sigma*sigma);
            
            TPZVec<TVar> xc(dim);
            
            TPZFMatrix<TVar> xdiff(dim,1);
            for(int i=0; i<dim; i++){
                xc[i] = 0.5;
                xdiff(i,0) = x[i]-xc[i];
            }
//            TPZFMatrix<TVar> P = {{-1./3.,2./3.,-2./3.},
//                                  {-2./3.,1./3.,2./3.},
//                                  {-2./3.,-2./3.,-1./3.}};
//
//            TPZFMatrix<TVar> PInv(3,3);
//            PInv.Identity();
//            P.Solve_LU(&PInv);
//
//            TPZFMatrix<TVar> D = {{0.3,0.,0.},{0.,0.001,0.},{0.,0.,0.00001}};
//            TPZFMatrix<TVar> Ktemp(3,3,0),K(3,3,0);;
//            P.Multiply(D, Ktemp);
//            Ktemp.Multiply(PInv, K);
            ...
            for(int i=0; i<dim; i++){
                exp += xdiff(i,0)*xdiff(i,0);
            }
            
            exp /= (-(TVar)(dim)*sigma*sigma);
            exp = pow((TVar)(std::exp(1.)),exp);
            
//            TPZFMatrix<TVar> xdiffT = xdiff;
//
//            xdiffT.Transpose();
//            TPZFMatrix<TVar> auxMat(1,3), auxMat2(1,1);
//            xdiffT.Multiply(K, auxMat);
//            auxMat.Multiply(xdiff, auxMat2);
//
//            const TVar coef2 = auxMat2(0,0);
//
//            TVar coef3;
//            K.Diagonal(coef3);
//
//            TVar coef = coef1*(-coef1*coef2+coef3);
...
//                x0[i] = TVar(-0.05);
        ...
        case EAnisoPerm:
        {
            TVar sigma = (TVar)(std::sqrt(0.005));
            TVar center = (TVar)(0.5);
            TVar exp = (TVar)(0.);
            for(int i=0; i<this->fDimension; i++){
                exp += (xloc[i]-center)*(xloc[i]-center);
            }
            
            exp /= (TVar)(-(TVar)(fDimension))*sigma*sigma;
            exp = FADexp(exp);
...
void TLaplaceExample1::SetPermeabilyTensor(const REAL &perm){
    const int dim = 3;
    fTensorPerm.Resize(dim, dim);
    fInvPerm.Resize(dim, dim);
    fTensorPerm.Zero();
    fInvPerm.Zero();
...
void TLaplaceExample1::SetPermeabilyTensor(TPZFNMatrix<9,REAL> K, TPZFNMatrix<9,REAL> invK){
    ...
    for(int irow=0; irow<dim; irow++){
        for(int icol=0; icol<dim; icol++){
            fTensorPerm(irow,icol) = K(irow,icol);
            fInvPerm(irow,icol) = invK(irow,icol);
        }
    }
}

void TLaplaceExample1::GetPermeabilyTensor(TPZFMatrix<STATE> &perm, TPZFMatrix<STATE> &invperm){
    const int dim = fTensorPerm.Rows();
    perm.Resize(dim, dim);
    invperm.Resize(dim, dim);
    
    for(int irow=0; irow<dim; irow++){
        for(int icol=0; icol<dim; icol++){
            perm(irow,icol) = fTensorPerm(irow,icol);
            invperm(irow,icol) =fInvPerm(irow,icol);
        }
    }
}
...
void TLaplaceExample1::SigmaLoc(const TPZVec<TVar> &x, TPZFMatrix<TVar> &sigma) const
{
    ...
    for(int irow=0; irow<dim; irow++){
        for(int icol=0; icol<dim; icol++){
            sigma(irow) -= (TVar)(fTensorPerm(irow,icol))*grad[icol];
        }
    }
