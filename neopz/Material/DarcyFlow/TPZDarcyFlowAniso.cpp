...
TPZDarcyFlowAniso::TPZDarcyFlowAniso() : TPZRegisterClassId(&TPZDarcyFlowAniso::ClassId),
                               TBase(), fDim(-1) {}

TPZDarcyFlowAniso::TPZDarcyFlowAniso(int id, int dim) : TPZRegisterClassId(&TPZDarcyFlowAniso::ClassId),
                                              TBase(id), fDim(dim) {
                                              }

TPZDarcyFlowAniso::TPZDarcyFlowAniso(const TPZDarcyFlowAniso &copy) : TPZMatBase(copy), fDim(copy.fDim)
{
}

TPZDarcyFlowAniso& TPZDarcyFlowAniso::operator=(const TPZDarcyFlowAniso &copy) {
    TPZMatBase::operator=(copy);
    fDim = copy.fDim;
    return *this;
}

void TPZDarcyFlowAniso::SetDimension(int dim) {
    if (dim > 3 || dim < 1) DebugStop();
    fDim = dim;
}

void TPZDarcyFlowAniso::Contribute(const TPZMaterialDataT<STATE> &data, STATE weight, TPZFMatrix<STATE> &ek,
                              TPZFMatrix<STATE> &ef) {
...
void TPZDarcyFlowAniso::ContributeBC(const TPZMaterialDataT<STATE> &data, STATE weight, TPZFMatrix<STATE> &ek,
...
int TPZDarcyFlowAniso::VariableIndex(const std::string &name) const {
...
int TPZDarcyFlowAniso::NSolutionVariables(int var) const {
...
void TPZDarcyFlowAniso::Solution(const TPZMaterialDataT<STATE> &data, int var, TPZVec<STATE> &solOut) {
...
void TPZDarcyFlowAniso::GetSolDimensions(uint64_t &u_len, uint64_t &du_row, uint64_t &du_col) const {
    u_len=1;
    du_row=fDim;
    du_col=1;
}

void TPZDarcyFlowAniso::Errors(const TPZMaterialDataT<STATE> &data,
                          TPZVec<REAL> &errors) {
    const TPZVec<REAL> &x = data.x;
...
int TPZDarcyFlowAniso::ClassId() const {
    return Hash("TPZDarcyFlowAniso") ^ TBase::ClassId() << 1;
}

TPZMaterial *TPZDarcyFlowAniso::NewMaterial() const {
    return new TPZDarcyFlowAniso(*this);
}

void TPZDarcyFlowAniso::Print(std::ostream &out) const {
    out << "Material Name: " << this->Name() << "\n";
    out << "Material Id: " << this->Id() << "\n";
    out << "Dimension: " << this->Dimension() << "\n\n";
}

void TPZDarcyFlowAniso::FillDataRequirements(TPZMaterialData &data) const {
    data.SetAllRequirements(false);
}


void TPZDarcyFlowAniso::FillBoundaryConditionDataRequirements(int type, TPZMaterialData &data) const {
